...
ori_img = Image.open("posImg/FudanPed00001.png")
...
kmeans_df["Color Name"] = list(
    map(get_colour_name, np.uint8(kmeans.cluster_centers_)))
...
def calculateBCSS(X, kmeans):
    ...
    diff_cluster_sq = np.linalg.norm(
        kmeans.cluster_centers_ - np.mean(X, axis=0), axis=1)**2
...
def locateOptimalElbow(x, y):
    ...
    a, b = m, -1
    dist = np.array([abs(a*x0+b*y0+c)/math.sqrt(a**2+b**2)
...
for col in kmeans_result.columns[2:]:
    ...
    if col == "Image Size (KB)":
        ...
        optimal_k_dict["Optimal k"] = locateDrasticChange(
            kmeans_result.index, kmeans_result[col].values)
    else:
        optimal_k_dict["Method"] = "Elbow"
        optimal_k_dict["Optimal k"] = locateOptimalElbow(
            kmeans_result.index, kmeans_result[col].values)
